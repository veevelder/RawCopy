					...
					if(hDrive != INVALID_HANDLE_VALUE) {
						...
						if(DeviceIoControl(hDrive, FSCTL_GET_NTFS_VOLUME_DATA, NULL, 0, &ntfsVolData, sizeof(ntfsVolData), &dwWritten, NULL)) {
							...
							if(GetFileInformationByHandle(hFile, &bhFileInfo)) {
								...
								NTFS_FILE_RECORD_INPUT_BUFFER mftRecordInput = {{bhFileInfo.nFileIndexLow, bhFileInfo.nFileIndexHigh}};					//set the location of the file record based on the file information
